...
\begin{tikzpicture}
  ...
  \foreach \n in {0,...,3}
  {
    ...
    \node [draw, align=center, above, fill=blue, text=white, minimum height=1cm] at($(node_0.south)+(0,.5)$) (mpirun_call_0) {$12x$\\$\texttt{Lola's files}$};
    \path [thick,color=blue] (mpirun_call_0) edge (node_0.south) 
    (node_0.south) edge (switch);

    \foreach \x in {2,...,3}{
      \node [draw,fill=blue,rectangle,minimum width=2mm,minimum height=2mm,anchor=north west] at(node_\x_core_\x_1.north west) (busy_a_core_\x) {};
      \node [draw,fill=blue,rectangle,minimum width=2mm,minimum height=2mm,anchor=north west] at(node_\x_core_\x_2.north west) (busy_b_core_\x) {};
      \node [draw, align=center, above, fill=blue, text=white, minimum height=1cm] at($(node_\x.south)+(0,.5)$) (mpirun_call_\x) {$2x$\\$\texttt{Lola's files}$};

      \path [thick,color=blue] (mpirun_call_\x) edge (node_\x.south) 
    (node_\x.south) edge (switch);
    ...
    \foreach \x in {0,...,3}{
      \node [draw,fill=red,rectangle,minimum width=2mm,minimum height=5mm,anchor=north east] at(node_\x_core_0_0.north east) (heartbeat_1_core_\x) {};
    }



    \node [draw, align=center, above, fill=blue, text=white, minimum height=1cm] at($(node_1.south)+(0,.5)$) (mpirun_call_1) {$4x$\\$\texttt{Lola's files}$};
